class Context:
    ...
    def confounded_model(self, p_confounder: float):
        ...
        if p_confounder <=0 or p_confounder > 1:
            raise ValueError("The value of p_confounder must be in the range (0, 1]" +
                             f" Instead, got p_confounder={p_confounder}.")
        self._assumptions.append("confounded")
    ...
    def unfaithful_model(self, p_unfaithful: float):
        ...
        if p_unfaithful <=0 or p_unfaithful > 1:
            raise ValueError("The value of p_unfaithful must be in the range (0, 1]" +
                             f" Instead, got p_unfaithful={p_unfaithful}.")
        self._assumptions.append("unfaithful")
    ...
    def autoregressive_model(self, order: int):
        ...
        """
        if order <=0:
            raise ValueError("The value of order must be an integer larger than 0." +
                             f" Instead, got order={order}.")
        self._assumptions.append("autoregressive")
    ...
    def measure_err_model(self, gamma: float):
        ...
        """
        if gamma <=0:
            raise ValueError("The value of gamma must be larger than 0." +
                             f" Instead, got gamma={gamma}.")
        self._assumptions.append("measurement_error")
    ...
    def p_confounder(self):
        if self._p_confounder is None:
            raise AttributeError("p_confounder is None. If you want a structural causal" +
    ...
    def p_unfaithful(self):
        if self._p_unfaithful is None:
            raise AttributeError("p_unfaithful is None. If you want a structural causal" +
    ...
    def measure_error_gamma(self):
        if self._measure_error_gamma is None:
            raise AttributeError("measure_error_gamma is None. If you want a structural causal" +
    ...
    def autoregressive_order(self):
        if self._autoregressive_order is None:
            raise AttributeError("autoregressive_order is None. If you want a structural causal" +
