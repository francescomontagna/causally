import torch
import numpy as np
from causally.datasets.causal_mechanisms import LinearMechanism, NeuralNetMechanism
...
def test_given_linear_mechanism_when_predict_then_output_is_linear_reg():
    ...
    assert all(y == (X*linear_coeff).squeeze()),\
...
def test_given_linear_mechanisms_then_weights_in_required_range():
    ...
    assert all(linear_coeffs < 1) and all(linear_coeffs > -1) and all(abs(linear_coeffs) > 0.05),\
...
def test_given_nn_mechanisms_then_weights_mean_zero_and_std_one():
    ...
    assert np.allclose(weights.mean(), 0, atol=0.05),\
        f"Empirical mean is {weights.mean()}, expected closer to 0."
    assert np.allclose(weights.std(), 1, atol=0.05),\
