...
from causally.graph.random_graph import (
    ErdosRenyi,
    BarabasiAlbert,
    ...
    for num_nodes in range(10, 100):
        ...
        if graph_type == "ER":
            generator = ErdosRenyi(
                num_nodes=num_nodes, expected_degree=int(np.random.choice(range(1, 4)))
            )
        if graph_type == "SF":
...
def test_er_has_expected_number_of_nodes():
    ...
    for num_nodes in nodes_values:
        ...
        A = generator.get_random_graph()
        assert num_nodes == A.shape[0], (
            "ER graph generated with wrong number of nodes"
...
def test_sf_has_expected_number_of_nodes():
    ...
    nodes_values = [5, 10, 20, 50]
    for num_nodes in nodes_values:
        generator = BarabasiAlbert(num_nodes=num_nodes, expected_degree=1)
        A = generator.get_random_graph()
        assert num_nodes == A.shape[0], (
            "SF graph generated with wrong number of nodes"
            f"Expected {num_nodes} nodes, got {A.shape[0]} instead."
        )
...
def test_grp_has_expected_number_of_nodes():
    ...
    for num_nodes in nodes_values:
        generator = GaussianRandomPartition(
            num_nodes=num_nodes, p_in=0.1, p_out=0.4, n_clusters=2
        )
        A = generator.get_random_graph()
        assert num_nodes == A.shape[0], (
            "GRP graph generated with wrong number of nodes"
            f"Expected {num_nodes} nodes, got {A.shape[0]} instead."
        )
...
def test_er_small_sparse_number_of_edges_and_degree_regularity():
    ...
    generator = ErdosRenyi(num_nodes=num_nodes, p_edge=0.1)
    ...
    assert (
        np.mean(
            abs(
                np.array(logs["max_income_degree"])
                - np.array(logs["max_outcome_degree"])
            )
        )
        <= 1
...
def test_er_medium_sparse_number_of_edges_and_degree_regularity():
    ...
    # Construct ER generator
    generator = ErdosRenyi(num_nodes=num_nodes, expected_degree=1)
    ...
    assert (
        np.max(logs["num_edges"]) >= generator.expected_degree * num_nodes - 1
        and np.max(logs["num_edges"]) <= generator.expected_degree * num_nodes + 1
    )  # max edges
    assert (
        np.min(logs["num_edges"]) >= generator.expected_degree * num_nodes - 1
        and np.min(logs["num_edges"]) <= generator.expected_degree * num_nodes + 1
    )  # min edges
    assert (
        np.mean(
            abs(
...
def test_er_medium_dense_number_of_edges_and_degree_regularity():
    ...
    # Construct ER generator
    generator = ErdosRenyi(num_nodes=num_nodes, expected_degree=2)
...
def test_er_large_dense_number_of_edges_and_degree_regularity():
    ...
    # Construct ER generator
    generator = ErdosRenyi(num_nodes=num_nodes, expected_degree=8)
...
def test_sf_medium_sparse_irregular_degree():
    ...
    avg_in_out_diff = np.mean(
        abs(np.array(logs["max_income_degree"]) - np.array(logs["max_outcome_degree"]))
    )
    assert avg_in_out_diff >= 3, (
        f"Mean difference between max input and output degree: {avg_in_out_diff}."
        " SF graphs shouldn't be regular!"
    )  # irregular graph
...
def test_sf_large_sparse_irregular_degree():
    ...
    # Construct SF generator
    generator = BarabasiAlbert(num_nodes=num_nodes, expected_degree=2)
...
def test_gpr_has_expected_number_of_clusters():
    ...
    for num_nodes in nodes_values:
        ...
        assert len(generator.size_of_clusters) == n_clusters, (
...
def test_grp_clusters_er_regular_degree_behaviour():
    ...
    for num_nodes in nodes_values:
        ...
        for _ in generator.size_of_clusters:
            ...
            max_outcome_degree = np.max(A.sum(axis=1))
            logs["max_income_degree"] = logs["max_income_degree"] + [max_income_degree]
            logs["max_outcome_degree"] = logs["max_outcome_degree"] + [
                max_outcome_degree
            ]
